...
%          vwaps=cell(1,zongshu);
 ...
 for i=1:zongshu
    ...
    if(datao(end,j)*datav(end,j)>800000 && datao(end,j)>10)
...
title('Backtesting simulation of AATS Strategy')
 ...
 Sharpe=15.8*a/b;
  disp(['Sharpe ratio of the past 500 days: ', num2str(Sharpe)]);
%   disp(['Standard Deviation of 10K book value: ', num2str(std(meiriPL((end-500):end)/(duishu*2)))]);
 
 
 
 
 
    disp(['Average daily profit of one dollar in the past 500 days: ', num2str(( PL(end-400)-PL(end-600))/(200*duishu*2))]);
...
%              end
 ...
 disp(['Maximum of nonprofit interval: ', num2str(buzhuan)]);
 ...
 for i=750:(L-l)
  ...
  disp(['Number of more than ', num2str(lost),' continous lost days: ', num2str(count)]);
 ...
 disp(['Days that per 1 dollar book value profit smaller than ', num2str(limit),': ', num2str(c)]);
  disp(['Minimal profit 1 dollar book value profit: ', num2str( zuixiao/(duishu*2))]);
