...
%  tradedSymbols=cell(1,31);
 for i=1:zongshu
     ...
     if(datao(end,j)*datav(end,j)>20000000 && datao(end,j)>100)
...
%   cell2csv('C:\Company\tradedSymbols2.csv', tradedSymbols,',');
 zongshu=j-1;
 ...
 for i=1:(L-l)
    ...
    pcData(L-l+1-i,j)=datac(L-i+1,j)/datac(L-i-l+1,j)-1; 
        ...
        zuida=datah(L-i+1,j);
        zuixiao=datal(L-i+1,j);
        for t=1:(l-1)
            zuida=max(datah(L-i+1-t,j),zuida);
            zuixiao=min(datal(L-i+1-t,j),zuixiao);
        end
       
    if(zuida-zuixiao ~=0)
    pchlData(L-l+1-i,j)=(datac(L-i+1,j)-zuixiao)/(zuida-zuixiao); 
    else
     pchlData(L-l+1-i,j)=0.5;   
    end
    pchData(L-l+1-i,j)=datac(L-i+1,j)/zuida-1; 
    pclData(L-l+1-i,j)=datac(L-i+1,j)/zuixiao-1; 
    
    
        zuida=datah(L-i,j);
        zuixiao=datal(L-i,j);
        for t=1:(l-1)
            zuida=max(datah(L-i-t,j),zuida);
            zuixiao=min(datal(L-i-t,j),zuixiao);
        end
    
    if((datah(L-i-l+1,j)-datal(L-i-l+1,j))~=0)
    pohlData(L-l+1-i,j)=(datao(L-i+1,j)-zuixiao)/(zuida-zuixiao); 
    else
    ...
    pohData(L-l+1-i,j)=datao(L-i+1,j)/zuida-1; 
    polData(L-l+1-i,j)=datao(L-i+1,j)/zuixiao-1; 
...
%        end
 %    if(newPositions(j) ~=positions(j))
   if(sum(abs(positions))>7 || (positions(j)==0 && newPositions(j) ~=0 )|| (abs(newPositions(j)-positions(j))==2))
...
%   if(newPositions(j) ~=positions(j))
 ...
 Sharpe=15.8*a/b
%  
%  ( PL(end)-PL(end-250))/(250*duishu*2)
   ( PL(end)-PL(end-250))/(250*8)
