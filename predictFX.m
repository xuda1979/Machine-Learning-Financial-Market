...
targetSeries = tonndata(shuchu(1:(N-L),j:j),false,false);
...
targetSeries = tonndata(shuchu((1+50*(i)):(N-L+50*(i)),j:j),false,false);
...
eval([v '((50*i-49):(50*i))=(transpose(ys((end-49):end))-shuchu((N-L+50*i-49):(N-L+50*i),j:j))./DataOpen((N-L+50*i-49):(N-L+50*i),j:j);']);
...
eval([gains '=shuchu((end-L+1):end,j:j)./DataOpen((end-L+1):end,j:j)-1;']); 
eval([FXnet '=net;']);
eval(['save ' v]);
eval(['save ' gains]);
eval(['save ' FXnet]);
